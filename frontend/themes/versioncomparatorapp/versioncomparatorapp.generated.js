...
window.Vaadin.Flow['_vaadintheme_versioncomparatorapp_globalCss'] = window.Vaadin.Flow['_vaadintheme_versioncomparatorapp_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_versioncomparatorapp_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_versioncomparatorapp_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_versioncomparatorapp_globalCss'].push(target);
  }
  if (!document['_vaadintheme_versioncomparatorapp_componentCss']) {
    
    document['_vaadintheme_versioncomparatorapp_componentCss'] = true;
  }
